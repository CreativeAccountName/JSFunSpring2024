(function () {
	...
	const handleSubmit = (e) => {
		...
		const results = `<p class="italic">No results for "${ocean}" found.</p>`;
		searchCont.insertAdjacentHTML("beforeend", results);
	};
	...
	const movie = {
		title: "Luca",
		year: 2021,
		imgSrc: "https://m.media-amazon.com/images/M/MV5BNWMzZjNjMjgtYjdlNS00ZmNiLThlYWQtOTkyNmNmZjBlOTdhXkEyXkFqcGdeQXVyODk4OTc3MTY@._V1_QL75_UX280_CR0,0,280,280_.jpg",
		description:
  ...
  <div class="flex-auto my-4">
	...
	// Write you JavaScript here
	const movieInfo = document.querySelector(".grid.grid-flow-col.gap-4");
	movieInfo.insertAdjacentHTML("beforeend", movieHtml);
	...
	const conversationDialogues = [
		...
		{
			author: "You",
			message: "When would I want to use an array and when would I want to use an object?",
		},
		{
			author: "ChatGPT",
			message: "Deciding whether to use an array or an object in JavaScript depends on the structure and purpose of your data. Here are some guidelines to help you decide: ...",
		},
	];
   ...
   */
	...
	for (const dialogue of conversationDialogues) {
		messageBox = `<div class="border-solid border-2 border-slate-200 rounded p-2 mb-4">
      <div class="font-bold">${dialogue.author}</div>
      <p>${dialogue.message}</p>
    </div>`;
		convo.insertAdjacentHTML("beforeend", messageBox);
	}
})();
